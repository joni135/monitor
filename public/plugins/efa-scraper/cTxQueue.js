...
var cTxQueue = {
	...
	buildContainer : function(txs) {
		...
		var base64api = base64.replace(/\//g, '-').replace(/\+/g, '*').replace(
				/\=/g, '_');
		console.log("xACtaq|V%1 container built: %2 ...", this.apiVersion,
	...
	addNewTxToPending : function(type, tablename, record, retries, callback, onError) {
		...
		console.log("c7OduW|Pending queue" + " ++: " + cTxQueue.queues.txPending.length);
		return tx;
	},
	...
	shiftTransactions : function(source, destination, actionToRegister, count) {
		...
		while ((sourceQ.length > 0) && ((i < count) || (i == 0))) {
			...
			if (actionToRegister == this.ACTION_TX_SEND) {
				...
				txShifted.resultMessage = "3nigIB|No server response in re...";
				txShifted.retries++;
				txShifted.closedAt = Date.now();
			} else if (actionToRegister == this.ACTION_TX_CONTAINER_FAILED) {
				txShifted.resultAt = Date.now();
				txShifted.resultCode = 504;
				txShifted.resultMessage = "7dNVv9|Transaction response con...";
				txShifted.closedAt = Date.now();
			}
	...
	onRetryTimerEvent : function() {
		cTxQueue.millisToNextRetry = 600000;
	...
	parseResponse : function(response) {
		...
		if (txc.cApiVersion > this.apiMaxVersion) {
			...
			txc.cresultMessage = "ui3jKY|WARNING: API version of ...",
	...
	sendTxs : function(txs) {
		...
		postRequest.onload = function() {
			...
			if (postRequest.status == 500) {
				...
				txrc.resultCode = 500;
				txrc.resultMessage = "EJM8uI|Server returned status c...";
                console.log(txrc)
		...
		postRequest.onerror = function() {
			txrc = Object.assign({}, cTxQueue.emptyContainer);
			txrc.resultCode = 500;
			txrc.resultMessage = "cbHhdY|Client post request erro...";
            console.log(txrc)
			cTxHandler.handleContainerError(txrc);
		};
		...
		postRequest.ontimeout = function() {
			txrc = Object.assign({}, cTxQueue.emptyContainer);
			txrc.resultCode = 500;
			txrc.resultMessage = "8ySDU0|Post request time out";
            console.log(txrc)
			cTxHandler.handleContainerError(txrc);
		};
	...
	txToString : function(tx) {
		...
		txStr += "YcDS4y|result: " + tx.resultCode + " " + messageStr + " ";
		if (tx.retries > 0)
			txStr += "qZaY4N|(needed %1 retries)", tx.retries;
		var closedStr = (tx.closedAt == 0) ? "ncPBhO|not yet " : closedDate
				.toLocaleDateString($_locale, $_dateFormatDayShort)
				+ " " + closedDate.toLocaleTimeString();
		txStr += "ofYJXP|closed: " + closedStr + " ";
		return txStr;
	},
		...
		for (txr of txrc.txrs) {
			...
			if (tx) {
				if (parseInt(tx.ID) != parseInt(txr.ID)) {
					...
					tx.resultMessage = "oaIKyl|Returned transaction is ...", txr.ID, tx.ID;
					tx.resultAt = now;
					tx.closedAt = now;
				} else {
					...
					if ((tx.resultCode >= 400) || (tx.resultCode == 304)) {
						...
						// not checked
						if (!tx.resultMessage)
							tx.resultMessage = $_resultMessages[tx.resultCode];
						...
						if (typeof cTxHandler[resp_f] === "function")
							...
							tx.resultCode = 501;
							tx.resultMessage = "x9Civl|Invalid type Â°%1Â° in ser...", tx.type;
						}
					}
...
var txRetryTimer = setInterval(cTxQueue.onRetryTimerEvent, 600000);